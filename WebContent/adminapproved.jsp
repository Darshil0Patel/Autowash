<!doctype html>
<%@page import="com.Bean.approved"%>
<%@page import="com.dao.CustomerDao"%>
<%@page import="com.Bean.pending"%>
...
<body>
    ...
    <%
   		List<approved> list = AdminDao.showapprovedlist();
   		Admin a3 = null;
                        ...
                        <div class="card mb-3">
                            <div class="card-header-tab card-header">
                                <div class="card-header-title font-size-lg text-capitalize font-weight-normal"><i
                                        class="header-icon lnr-laptop-phone mr-3 text-muted opacity-6"> </i>Pending Services
                                </div>
                                ...
                                <table style="width: 100%;" id="example" class="table table-hover table-striped table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td><%=p.getCid() %></td>
                                            <td><%=p.getSid() %></td>
                                            <td><%=p.getCfname()+" "+p.getClname() %></td>
                                            <td><%=p.getSfname()+" "+p.getSlname() %></td>
                                            <td><%=p.getSemail() %></td>
                                            <td><%=p.getServiceID() %></td>
                                            <td><%=p.getService() %></td>
                                            <td>
                                            <form style="text-align: center;" action="AdminController"  method="post">
                                            <input type = "hidden" name="id" value=<%=p.getId() %> >
                                            <input name="cid" value=<%=p.getCid() %> type="hidden">
											<input name="sid" value=<%=p.getSid() %> type="hidden">
											<input name="cfname" value=<%=p.getCfname() %> type="hidden">
											<input name="sfname" value=<%=p.getSfname() %> type="hidden">
											<input name="clname" value=<%=p.getClname() %> type="hidden">
											<input name="slname" value=<%=p.getSlname() %> type="hidden">
											<input name="cemail" value=<%=p.getCemail() %> type="hidden">
											<input name="semail" value=<%=p.getSemail() %> type="hidden">
											<input name="ServiceID" value=<%=p.getServiceID() %> type="hidden">
											<input name="service" value=<%=p.getService() %> type="hidden">
                                            <button  name="action" value="pending" class="btn btn-success">Pending</button>
                                            <!-- <button name="action" value="updatecustomerpage">Update</button> -->
                                            </form>
                                            </td> 
                                        </tr>
